...
export const blankRatings = { taste: 4, service: 4, atmosphere: 4, value: 4, tortilla: 4, salsa: 4, filling: 4, egg: 4, cheese: 4, balance: 4 } as Record<OfficialKey | BreakdownKey, number>;
...
export function groupByRestaurant(reviews: Review[]) {
  ...
  return [...map.entries()].map(([name, items]) => {
    ...
    return { name, reviews: items, count: items.length, overall: avg('overall'), taste: avg('taste'), service: avg('service'), atmosphere: avg('atmosphere'), value: avg('value'), tortilla: avg('tortilla'), salsa: avg('salsa'), filling: avg('filling'), egg: avg('egg'), cheese: avg('cheese'), balance: avg('balance'), awards };

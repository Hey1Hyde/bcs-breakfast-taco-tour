  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top,#F8DDA4_0,#FFF7E8_36%,#FFF7E8_100%)] pb-28">
    ...
    <section className="mx-auto max-w-md space-y-4 p-4">
      ...
      {tab==='score' && <Card title="Score a restaurant" emoji="✍️"><div className="space-y-3">
        ...
        <button onClick={saveReview} disabled={isSaving} className={`pressable save-button w-full rounded-3xl p-5 text-lg font-black text-white shadow-soft disabled:opacity-80 ${saveComplete ? 'bg-salsa' : 'bg-orange'}`}>{isSaving ? <span className="inline-flex items-center justify-center gap-2"><span className="save-spinner" aria-hidden="true"/>Saving score…</span> : saveComplete ? '🌮 Review Saved!' : 'Save score 🌮'}</button>

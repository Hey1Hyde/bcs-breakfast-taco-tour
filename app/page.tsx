...
import { awardOptions, blankRatings, breakdownCategories, fmt, groupByRestaurant, normalizeReview, overallScore, Review } from '@/lib/tacoData';
...
export default function Home() {
  ...
  async function saveReview() {
    ...
    try {
      ...
      setActiveRestaurant(savedReview.restaurantName);
      setSaveComplete(true);
      setCelebration(Array.from({ length: 10 }, (_, index) => index));
      if ('vibrate' in navigator) navigator.vibrate(35);
      setTimeout(() => setCelebration([]), 1500);
      setTimeout(() => setTab('leaderboard'), 650);
      ...
      setIsSaving(false);
      setTimeout(() => setSaveComplete(false), 1600);
    }
  }
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top,#F8DDA4_0,#FFF7E8_36%,#FFF7E8_100%)] pb-28">
    ...
    <section className="mx-auto max-w-md space-y-4 p-4">
      ...
      {tab==='score' && <Card title="Score a restaurant" emoji="✍️"><div className="space-y-3">
        ...
        <h3 className="font-black">Taco ratings <span className="text-sm text-bean/60">Overall: {fmt(overallScore(form))}</span></h3><ScoreMeter value={overallScore(form)} />{breakdownCategories.map(c=><Slider key={c.key} label={`${c.emoji} ${c.label}`} value={form[c.key]} onChange={v=>update(c.key,v)}/>) }
        <h3 className="font-black">Awards / tags</h3><div className="flex flex-wrap gap-2">{awardOptions.map(a=><button key={a} onClick={()=>update('awards', form.awards.includes(a) ? form.awards.filter(x=>x!==a) : [...form.awards,a])} className={`pressable rounded-full px-3 py-2 text-sm font-bold ${form.awards.includes(a)?'bg-orange text-white':'bg-white'}`}>{a}</button>)}</div>
        <div className="relative"><button onClick={saveReview} disabled={isSaving} className={`pressable save-button w-full rounded-3xl p-5 text-lg font-black text-white shadow-soft disabled:opacity-80 ${saveComplete ? 'bg-salsa' : 'bg-orange'}`}>{isSaving ? <span className="inline-flex items-center justify-center gap-2"><span className="save-spinner" aria-hidden="true"/>Saving score…</span> : saveComplete ? '🌮 Review Saved!' : 'Save score 🌮'}</button>{celebration.length > 0 && <div className="taco-celebration" aria-hidden="true">{celebration.map(index => <span key={index} className="taco-confetti" style={{ '--taco-left': `${10 + index * 8}%` } as React.CSSProperties}>🌮</span>)}</div>}</div>
        <div aria-live="polite" className={`success-toast rounded-2xl bg-white p-3 text-center font-black text-salsa shadow-soft ${saveComplete ? 'success-toast--show' : ''}`}>🌮 Review Saved!</div>
      </div></Card>}
      {tab==='leaderboard' && <Card title="Leaderboard" emoji="🏆"><div className="space-y-3">{restaurants.map((r,i)=><button key={r.name} onClick={()=>{setActiveRestaurant(r.name);setTab('restaurants')}} className="pressable stagger-card w-full rounded-3xl bg-white p-4 text-left shadow-soft" style={{ animationDelay: `${Math.min(i * 45, 240)}ms` }}><div className="flex items-center gap-3"><span className="text-3xl">{i===0?'🥇':i===1?'🥈':i===2?'🥉':`#${i+1}`}</span><div className="flex-1"><b>{r.name}</b><p className="text-sm text-bean/60">{r.count} review{r.count>1?'s':''} • Tortilla {fmt(r.tortilla)} • Salsa {fmt(r.salsa)} • Filling {fmt(r.filling)}</p></div><div className="w-20 text-right"><span className="text-xl font-black text-orange">{fmt(r.overall)}</span><ScoreMeter value={r.overall} compact /></div></div></button>)}</div></Card>}
      ...
      {tab==='awards' && <Card title="Awards board" emoji="🏅"><div className="space-y-3">{awardOptions.map(a=>{const winners=restaurants.filter(r=>r.awards.includes(a)); return <div key={a} className="award-badge rounded-2xl bg-white p-3"><b>{a}</b><p className="text-sm text-bean/70">{winners.map(w=>w.name).join(', ') || 'No winner yet'}</p></div>})}</div></Card>}
      {tab==='settings' && <Card title="Settings & backup" emoji="⚙️"><p className="mb-3 text-sm">Data saves to the shared Supabase database. Export JSON whenever someone wants an extra backup.</p><a download="bcs-taco-tour-backup.json" href={exportHref} className="pressable block rounded-2xl bg-orange p-4 text-center font-black text-white">Export JSON</a><label className="mt-3 block rounded-2xl bg-white p-4 text-center font-black opacity-60">Import JSON (temporarily disabled)<input className="hidden" type="file" accept="application/json" disabled/></label><button disabled className="mt-3 w-full rounded-2xl bg-salsa p-4 font-black text-white opacity-60">Reset to starter data (temporarily disabled)</button></Card>}
      </div>
    </section><nav className="safe-bottom fixed bottom-0 left-0 right-0 z-30 border-t border-orange/10 bg-cream/95 backdrop-blur"><div className="mx-auto grid max-w-md grid-cols-5 gap-1 p-2">{tabs.map(t=><button key={t.id} onClick={()=>setTab(t.id)} className={`pressable rounded-2xl p-2 text-[11px] font-black transition-colors duration-200 ${tab===t.id?'bg-orange text-white':'text-bean'}`}><div className="text-lg">{t.icon}</div>{t.label}</button>)}</div></nav>
  </main>;
}
function Card({title,emoji,children}:{title:string;emoji:string;children:React.ReactNode}){return <div className="fade-card rounded-[2rem] border border-white/80 bg-white/70 p-4 shadow-soft"><h2 className="mb-3 text-xl font-black"><span>{emoji}</span> {title}</h2>{children}</div>}
...
function ScoreMeter({value, compact = false, variant = 'default'}:{value:number;compact?:boolean;variant?:'default'|'light'}){const width = `${Math.max(0, Math.min(100, ((value - 1) / 9) * 100))}%`; return <div className={`score-meter ${compact ? 'score-meter--compact' : ''} ${variant === 'light' ? 'score-meter--light' : ''}`} aria-label={`Score meter: ${fmt(value)} out of 10`}><span style={{ width }} /></div>}
...
function Profile({restaurant}:{restaurant: ReturnType<typeof groupByRestaurant>[number]}){return <div className="space-y-4"><div className="rounded-3xl bg-orange p-5 text-white"><p className="text-sm font-bold">Overall score</p><p className="text-5xl font-black">{fmt(restaurant.overall)}</p><ScoreMeter value={restaurant.overall} variant="light" /><p>{restaurant.count} review{restaurant.count>1?'s':''}</p></div><div className="grid grid-cols-2 gap-2">{breakdownCategories.map(c=><div key={c.key} className="stagger-card rounded-2xl bg-white p-3"><p className="text-xs font-bold">{c.emoji} {c.label}</p><b>{fmt(restaurant[c.key])}</b></div>)}</div><div className="flex flex-wrap gap-2">{restaurant.awards.map(a=><span key={a} className="award-badge rounded-full bg-gold px-3 py-1 text-sm font-black">{a}</span>)}</div><div className="space-y-2">{restaurant.reviews.map(r=><div key={r.id} className="stagger-card rounded-2xl bg-white p-3"><b>{r.reviewerName}</b> <span className="text-xs text-bean/60">{r.date}</span><p>{r.sentenceReview}</p>{r.memorableQuote&&<p className="mt-1 italic text-orange">“{r.memorableQuote}”</p>}<p className="text-xs text-bean/60">Ordered: {r.ordered || '—'} • Price: {r.price || '—'}</p></div>)}</div></div>}

  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top,#F8DDA4_0,#FFF7E8_36%,#FFF7E8_100%)] pb-28">
    ...
    <section className="mx-auto max-w-md space-y-4 p-4">
      ...
      {tab==='score' && <Card title="Score a restaurant" emoji="✍️"><div className="space-y-3">
        ...
        <h3 className="font-black">Official ratings <span className="text-sm text-bean/60">Overall: {fmt(overallScore(form))}</span></h3><ScoreMeter value={overallScore(form)} />{officialCategories.map(c=><Slider key={c.key} label={`${c.emoji} ${c.label} (${Math.round(c.weight*100)}%)`} value={form[c.key]} onChange={v=>update(c.key,v)}/>) }
